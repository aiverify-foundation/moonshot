...
def install_moonshot(version = version):
    ...
    if 'moonshot' not in installed_packages:
        ...
        # run_subprocess([sys.executable, '-m', 'pip', 'install', f'projectmoonshot-imda[all]=={version}'], check=True)
        run_subprocess([sys.executable, '-m', 'pip', 'install', '-i', 'https://test.pypi.org/simple/', '--extra-index-url', 'https://pypi.org/simple/', f'projectmoonshot-imda[all]=={version}'], check=True)
...
def ms_lib_env_file(dir):
    ...
    # Create DATABASES, RESULTS, RUNNERS folder.
    for folder in [f'{data_repo_name}/generated-outputs/databases', 
                   f'{data_repo_name}/generated-outputs/results', 
                   f'{data_repo_name}/generated-outputs/runners']:
...
def clone_and_install_repos(repositories, base_dir):
    ...
    for repo, branch_name in repositories:
        ...
        if os.path.exists(folder_name):
            print(f'Directory {folder_name} already exists, skipping clone and installation.')
        else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        action = sys.argv[1]
        if action == "web":
            lib_thread = threading.Thread(target=run_moonshot_lib, args=("web-api",)).start()
            ui_thread = threading.Thread(target=run_moonshot_ui_dev).start()
        elif action == "cli":
            cli_thread = threading.Thread(target=run_moonshot_lib, args=("cli",)).start()

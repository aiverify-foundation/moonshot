...
class BenchmarkTestState(BaseService):
    ...
    def add_task(self, id: str, task: asyncio.Task[Any], moonshot_runner: Runner) -> None:
        self.state[id] = {
            "async_task": task,
    ...
    def get_all_progress_status(self) -> dict[str, TestRunProgress]:
        return {id: task_info["status"] for id, task_info in self.state.items() if task_info["status"] is not None}

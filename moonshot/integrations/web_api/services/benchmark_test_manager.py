...
from ..types.types import BenchmarkCollectionType, CookbookTestRunProgress
...
class BenchmarkTestManager(BaseService):
    @inject
    def __init__(
        self, benchmark_test_state: BenchmarkTestState, webhook: Webhook
    ) -> None:
    ...
    async def cancel_task(self, executor_id: str) -> None:
        runner = moonshot_api.api_load_runner(executor_id)
        await runner.cancel()
    ...
    async def create_executor_and_execute(
        self,
        ...
        try:
            runner = self.create_runner(
                benchmark_input_data.run_name,
                benchmark_input_data.endpoints,
                self.webhook.on_executor_update,
            )
    ...
    def schedule_test_task(
        self,
        executor_input_data: BenchmarkRunnerDTO,
        benchmark_type: BenchmarkCollectionType,
    ) -> str:
        task_id = self.generate_unique_task_id()

        exec_benchmark_coroutine = self.create_executor_and_execute(
            executor_input_data, benchmark_type
        )

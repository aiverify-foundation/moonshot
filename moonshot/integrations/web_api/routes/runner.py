...
def get_runner_by_id(
    ...
    try:
        ...
        if e.error_code == "FileNotFound":
            raise HTTPException(
                status_code=404, detail=f"Failed to retrieve run: {e.msg}"
            )
        elif e.error_code == "ValidationError":
            raise HTTPException(
                status_code=400, detail=f"Failed to retrieve run: {e.msg}"
            )
        else:
            raise HTTPException(
                status_code=500, detail=f"Failed to retrieve run: {e.msg}"
            )
...
def delete_recipe(
    ...
    try:
        ...
        if e.error_code == "FileNotFound":
            raise HTTPException(
                status_code=404, detail=f"Failed to delete run: {e.msg}"
            )
        elif e.error_code == "ValidationError":
            raise HTTPException(
                status_code=400, detail=f"Failed to delete run: {e.msg}"
            )
        else:
            raise HTTPException(
                status_code=500, detail=f"Failed to delete run: {e.msg}"
            )

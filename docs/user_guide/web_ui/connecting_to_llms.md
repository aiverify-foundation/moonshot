...
2. Refer to the table below and provide the URI and/or Token as required.
   ...
   ||*Amazon Bedrock Endpoints in Moonshot require an additional parameter to be defined. Click on "More Configs" and enter the following under "Other Parameters", replacing `<YOUR-REGION-CODE>` with your model's [regional endpoint code](https://docs.aws.amazon.com/general/latest/gr/rande.html#regional-endpoints).|Other Parameters </br> ```"session": {"region-name": "<YOUR-REGION-CODE>"}```|
...
3. Select the connection type and refer to its respective section below for more info on its usage.
    ...
    | **URI**  (Required)               | It is recommended to give this a short placeholder value like `DEFAULT` for the URI to be automatically inferred based on your AWS environment setup. If you provide a URI of 8 characters or more, it'll be treated like specifying a boto3 `client.endpoint_url`.                                                                                                    | `DEFAULT`                              |
    | **Token** (Required)              | It is recommended to give this a short placeholder value like `NONE` and instead configure your AWS credentials via the environment (as standard for boto3 and AWS CLI) If you are unable to configure environment variables, refer to the section `session` in **Other Parameters** below. | `NONE`|
    ...
    | **Other Parameters** (Required)   | The Amazon Bedrock connector requires the configuration of an additional parameter `session`. </br> Under `session`, you need to define the `region_name`, which is the region code of your model endpoint. You can also define the folllowing optional parameters under `session`: </br> - `aws_access_key_id`: The access key to use when creating the client. This is entirely optional, and if not provided, the credentials configured for the session will automatically be used. You only need to provide this argument if you want to override the credentials used for this specific client. </br> - `aws_secret_access_key`: The secret key to use when creating the client. Same semantics as aws_access_key_id above. </br> - `aws_session_token`: The session token to use when creating the client. Same semantics as aws_access_key_id above. </br> The Amazon Bedrock connector also allows the configuration of the following optional parameters. </br> - `timeout`: The number of seconds before a connection timeout. </br> - `max_attempts`: The number of times Moonshot should retry an interaction with the endpoint. </br> - `temperature`: The amount of randomness. | ``` "params": { "timeout": 300, "max_attempts": 3, "temperature": 0.5, "session": {"region_name": "us-east-1", "aws_secret_access_key": "abc123", "aws_session_token": "456xyz"}} ``` |

...
from moonshot.api import api_get_all_results, api_read_results
from moonshot.interface.cli.benchmark.cookbook import generate_cookbook_table
from moonshot.interface.cli.benchmark.recipe import generate_recipe_table
...
def list_results() -> None:
    ...
    """
    results_list = api_get_all_results()
    if results_list:
...
def view_results(args) -> None:
    ...
    """
    results = api_read_results(args.results_filename)
    if not results:
        console.print("[red]There are no results found.[/red]")
        return
